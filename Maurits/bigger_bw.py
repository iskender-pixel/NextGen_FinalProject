...
for i in range(number_of_measurements):
    sig_tx = np.append(sig_tx, chirp_A)
...
# f0_2 = 1.5e9 + B
my_sdr, tddn = initialize_Pluto_TDD(Pluto_IP, fs, f0, rx_gain, tx_gain, samp_amount, B)
results = pluto_transmit_receive(my_sdr, tddn, tx_signal, 1, samp_amount,save_path)
rx_data=loadmat(f'{save_path}/received_data.mat')


time_new = np.arange(0, 2*T-1/fs, 1/fs)

rx_added = np.append(rx_added, rx_data['received_data'])
...
time_new = np.arange(0, 2*T-1/fs, 1/fs)
sig_mixer = np.exp(1j*2*np.pi*(0.5*k*time_new**2))

s_beat = rx_added * np.conj(sig_mixer)
nyq = 0.5 * fs 
normal_cutoff = B / nyq 
...
# time_axis_new = np.tile(time_new, number_of_measurements)
# fig, axs = plt.subplots(2, 2, figsize=(10, 8))
# axs[0,0].plot(time_new, np.real(sig_mixer))
# axs[0,0].set_title('Input signal')

# axs[0,1].plot(time_new, np.real(rx_added))
# axs[0,1].set_title("received signal")

# axs[1,0].plot(time_new, s_beat_filtered)
